...
The LocaInfra web dashboard (M3b): React 19 + Vite 8, Tailwind 4, shadcn (base-nova),
React Router 8 in Data Mode, TanStack Query, TanStack Form, nuqs, zustand, Eden Treaty.

```sh
bun run dev        # against a running LocaInfra server (proxied to 127.0.0.1:4488)
...
Skipped unless `LOCAINFRA_E2E=1`. The global setup starts `dist/locainfra` (or the CLI from
source) on port 4598 with `LOCAINFRA_HOME` in a scratch folder and a fixed session token, then the
...
remove everything with volumes (asserting no `li-m2-smoke*` containers, volumes or networks remain).
...
palette, and checks `locainfra.db` (0600) plus `locainfra env`. The M3b spec (project `m3b-smoke`)
...
LOCAINFRA_E2E=1 LOCAINFRA_E2E_CHANNEL=chrome bun run e2e  # chrome = installed Google Chrome
```

Optional: `LOCAINFRA_E2E_SCRATCH=<dir>` (scratch folder), `LOCAINFRA_E2E_BIN=<path>` (binary to
test), `LOCAINFRA_E2E_PORT=<port>` (default 4598).
