...
| `state/sqlite/` | `SqliteStateStore` (`locainfra.db`, SQLite WAL via `bun:sqlite` + Drizzle; `BEGIN IMMEDIATE` per update; migrations in `drizzle/`, applied on open; imports a legacy `state.json` once; mode 0600) | `StateReader`, `StateWriter` |
...
short-lived `li-snapshot-helper-*` containers.
...
- `DockerContainerExec.run` passes argv verbatim (never a shell). `timeoutMs` and the caller's signal abort the
  ...
  cannot kill an exec, so each exec carries a random `LOCAINFRA_EXEC_TAG` env var and a timed-out or capped process
...
- `DockerVolumeArchiver` pulls `alpine:3.20` when missing (pull output becomes `log` events), then runs a
  `--rm` helper labelled `io.locainfra.helper=snapshot`. `archive` mounts the volume read-only and writes a temp file
...
`SqliteStateStore` keeps machine-local state in `<stateDir>/locainfra.db`. The schema lives in
...
`locainfra.db.backup` (0600). A failed migration rolls back and fails with an `IO` error naming the backup.
...
- `data.live.test.ts` creates its own scratch `alpine` container (`--rm`, internal network) and volume, all named
  `li-test-*`, and removes them. It never touches other containers.
